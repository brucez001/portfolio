...
vec3 rotateMoon(vec3 p, float spin) {
  ...
  float cz = cos(-0.32), sz = sin(-0.32);
  p = vec3(p.x * cz - p.y * sz, p.x * sz + p.y * cz, p.z);
  float cx = cos(0.18), sx = sin(0.18);
...
void main() {
  ...
  albedo = mix(vec3(lum), albedo, 0.12);
  float h0 = height(vUv);
  ...
  vec3 col = albedo * uTint * (lam * uSun + uAmbient);
...
void main() {
  ...
  if (uLightTheme > 0.5) { col = uAccent; a *= 0.3; }
  a = clamp(a, 0.0, 0.85);
...
void main() {
  float s = aSeed;
  vec3 silver = uLightTheme > 0.5 ? vec3(0.22, 0.25, 0.34) : vec3(0.88, 0.9, 0.96);
  vec3 col = hash(s + 6.0) > 0.8 ? uAccent : silver;
